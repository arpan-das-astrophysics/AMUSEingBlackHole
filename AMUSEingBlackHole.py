...
def get_options():
    ...
    parser.add_argument("--mdot", type=float, help="Accretion rate in MSun/yr", default=.2e-6)
    ...
    parser.add_argument("-f", type=str, help="Outfile", default="/home/arpan/imfbondi/test")
...
class Potential(object):
    def __init__(self, R = 1.0 | units.parsec, M = 1.e5 | units.MSun):
        self.M = M
        self.R = R 
...
if __name__ == "__main__":
    ...
    dfprop = pd.DataFrame({"mass": [stars.mass.value_in(units.kg)], "radius": [stars.radius.value_in(units.m)], "mdot": [stars.mdot.value_in(units.MSun/units.yr)], "x": [stars.x.value_in(units.m)], "y": [stars.y.value_in(units.m)], "z": [stars.z.value_in(units.m)], "vx": [stars.vx.value_in(units.ms)], "vy": [stars.vy.value_in(units.ms)], "vz": [stars.vz.value_in(units.ms)]} )
    ...
    while t_evolve < t_end:
        ...
        for s in stars:
            ...
            if r2 < R2cut:
                ...
                if Mgas.value_in(units.MSun) >= dm.value_in(units.MSun):
                    s.vx *= (s.mass/(s.mass+dm))
                    s.vy *= (s.mass/(s.mass+dm))
                    s.vz *= (s.mass/(s.mass+dm))
                    s.mass += dm
                    ...
                    #Tkh = get_kelvin_helmholtz_timescale(s)
                    #dr = s.radius / Tkh * (t_evolve-t_prev)
                    #s.radius -= dr
                    #if s.radius.value_in(units.RSun) < 1.:
                    #    s.radius = 1. | units.RSun
                    #s.mdot = 0. | units.MSun/units.yr     
            else:
                s.mdot = 0. | units.MSun/units.yr
                #Tkh = get_kelvin_helmholtz_timescale(s)
        ...
        if t_evolve >= t_snapshot:
            ...
            dfprop = dfprop.append({"mass": stars.mass.value_in(units.kg), "radius": stars.radius.value_in(units.m), "mdot": stars.mdot.value_in(units.MSun/units.yr), "x": stars.x.value_in(units.m), "y": stars.y.value_in(units.m), "z": stars.z.value_in(units.m), "vx": stars.vx.value_in(units.ms), "vy": stars.vy.value_in(units.ms), "vz": stars.vz.value_in(units.ms)},ignore_index=True )
            ...
            if numStar0 > len(stars) and t_evolve/t_last_collision > 1.:
                print(t_evolve, '/', t_end, len(stars), stars.mass.max().value_in(units.MSun), stars.radius.max().value_in(units.RSun), stars.mass.sum().value_in(units.MSun), background_potential.M.value_in(units.MSun), mdot.value_in(units.MSun/units.yr), background_potential.R.value_in(units.parsec), (1./(t_evolve-t_last_collision))/((numStar0-len(stars))/t_last_collision))
            else:
                print(t_evolve, '/', t_end, len(stars), stars.mass.max().value_in(units.MSun), stars.radius.max().value_in(units.RSun), stars.mass.sum().value_in(units.MSun), background_potential.M.value_in(units.MSun), mdot.value_in(units.MSun/units.yr), background_potential.R.value_in(units.parsec))
